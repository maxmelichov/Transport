...
class App(object):
    def __init__(self, root):
        self.address1 =Text (root,height=1,width=20,font=("Helvetica", 10))
        self.address1.place(x=200,y=50)
        self.address1.pack()
        self.address2 = Text (root,height=1,width=20,font=("Helvetica", 10))
        self.address2.place(x=200,y=100)
        self.address2.pack()
        self.google_maps = GoogleMaps(api_key="your_google_maps_key")
        self.button = Button(root, text="Calculate", command=self.calculate_dis)
        self.button.place(x=170,y=60)
